...
const UniversityDetailEdit = () => {
  ...
  const handleModalAction = (action) => {
    if (!changed) {
      router.push(`/settings/universities/${university_id}`);
      return;
    }
    let content;
    if (action === "cancel") {
  ...
  const handleEditUniversity = async (values) => {
    ...
    try {
      const res = await client
        .post(`universities/${university_id}?_method=PUT`, { body: newUniversityInfo })
        .json();
      router.push(`/settings/universities/${university_id}`);
      showNotification({
        type: "success",
            ...
            <div className="col-span-8 col-start-5 px-9 mt-1 mb-6">
              {image ? (
                ...
                <div className="mt-1 transition-all relative">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src="/images/university-no-image.png"
                    width={160}
                    height={90}
                    style={{ objectFit: "contain" }}
                    alt=""
                    ...
                    <div className="pl-9 w-[240px]">
                      ...
                      >
                        <DatePicker
                          ...
                          picker="year"
                          className="w-[150px]"
                          value={value.year ? moment(value.year) : null}
                          allowClear={false}
                          onChange={(date) => {
                            const newItems = [...items];

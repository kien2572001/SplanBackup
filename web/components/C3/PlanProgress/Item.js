...
const Item = ({ type, planData, formData, setFormData, setChanged, errorMessage }) => {
    ...
    useEffect(() => {
        setCurrValue(planData?.real_amount)
    }, [formData, planData?.real_amount])

    return (
        <div className="items-center text-xl grid grid-cols-6 auto-rows-auto font-normal">
            <label htmlFor="real_amount" className="col-span-3 text-xl truncate">
            ...
            {type === "edit" ? (
               ...
               <>
                    <span className="col-start-5 text-right	">

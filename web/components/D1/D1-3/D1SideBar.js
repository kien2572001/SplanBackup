...
export default function D1SideBar() {
      ...
      try {
        ...
        if (res.success) {
          if (Array.isArray(res.data)) {
            // this case has no pagination so we can just get the docs list from res.data
            setDocsList(res.data);
          }
          else if (Array.isArray(res.data.data)) {
            // this case has pagination so res.data is pagination info
            // we need to get the docs list from res.data.data
            setDocsList(res.data.data);
          }
          else {
            // This lines is to prevent error
            setDocsList([]);
          }
        }
      } catch (error) {}
  ...
  return (
    <div className="w-[224px] min-h-screen bg-white shadow-md py-4">
      ...
      {docsList?.length === 0 && <NoData size="small" className="mt-8" />}
      <div
        ...
        {docsList?.map((doc) => {
          return (
            ...
            >
              ...
              <div className="text-[12px]">{doc?.category?.name}</div>
            </div>
          );
        })}
      </div>
